/* Original source: https://gist.github.com/averykhoo/8057a0324be445bd7eea32c89379eb76

I modified the code to support glitter color input and made some changes in formatting and functionality.*/
...
export function sparkleAnimation() {
    ...
    window.onload = function () {
        ...
        const scriptTag = document.querySelector("script[glitter-cursor]");
        if (scriptTag && scriptTag.getAttribute("glitter-cursor").toLowerCase() === "true") {
            animate_sparkles();
    ...
    function get_fixed_color() {
        const scriptTag = document.querySelector("script[glitter-color]");
        if (scriptTag) {
            const colorAttribute = scriptTag.getAttribute("glitter-color");
            if (colorAttribute) {
    ...
    function animate_sparkles(fps = 60) {
        ...
        setTimeout(function () { animate_sparkles(fps); }, interval_milliseconds);
    }
    ...
    function create_star(x, y, probability = 1.0) {
        ...
        if (min_index >= 0) {
            ...
            star[min_index].childNodes[0].style.backgroundColor =
                star[min_index].childNodes[1].style.backgroundColor = get_fixed_color();
